...
def main():
    ...
    parser.add_argument("--model_path", default='PATH_TO_TRAINED_METAMORPH_MODEL')
    parser.add_argument("--visualizer_path", default='PATH_TO_TRAINED_VISUALIZER')
    ...
    tokenizer, model, image_processor, context_len = load_metamorph_model(model_path=args.model_path)
    
    # Load visualization models only if needed
    pipeline, projector = None, None

    print("Loading visualization models...")
    pipeline, projector = load_visualization(visualizer_repo_id=args.visualizer_path)

    # Load input (image or video), generate images dont need to pass in any image
