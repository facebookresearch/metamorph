...
# Single-node debug script for Stage 1 Pretraining (adjust parameters as needed)
...
LR=1.87e-5             # Pretraining LR 
BATCH_SIZE=14          # Per device batch size
SAVE_STEPS=240000      # Save steps for debug script (adjust if needed for faster debugging)
NUM_TRAIN_EPOCHS=1

# Vision and model parameters
MM_PROJECTOR_TYPE="mlp2x_gelu"
FREEZE_VISION=True     # Vision tower frozen during connector pretraining
VISION_TOWER="siglip/CLIP-ViT-SO400M-14-384"
MODEL_VERSION="llama3" # Corresponds to PROMPT_VERSION
USE_VISION_AR=False    # From original debug script
NORMALIZE_VISION=True
VISION_COEF=1.0
# Note: Original script had 'True'. Stage 1 usually only trains the connector, keeping LLM frozen ('False').
TUNE_MM_MLP_ADAPTER=True # Set to 'True' ONLY if you intend to tune LLM during pretraining.
...
DATA_PATH="Path_To_PRETRAIN_Data_JSONL"               # Path to the pretraining data JSONL file
EXP_NAME="debug_pretrain_$(date +%Y%m%d_%H%M%S)"      # Unique experiment name for output
...
export WANDB_PROJECT='YOUR_WANDB_PROJECT_DEBUG' # Set your W&B project name (e.g., 'metamorph-debug-pretrain')
...
deepspeed --num_gpus=8 metamorph/train/train_mem.py \
    ...
    # Note: --pretrain_mm_mlp_adapter is intentionally omitted for pretraining stage

echo "Debug pretraining script finished."