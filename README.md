...
2. Install Packages
```Shell
conda env create -f metamorph_environment.yml
pip install -e .
```
...
**A:** Absolutely! You can substitute MetaCLIP with publicly available image-text datasets such as CC12M, LAION, or COYO. Our method does not rely on proprietary datasets. Just ensure you format the data into the instruction-tuning style pairs as described in the paper.
